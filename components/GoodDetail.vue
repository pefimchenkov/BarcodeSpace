<template>
  <section class="py-8 bg-white md:py-16 dark:bg-gray-900 antialiased">
    <div v-if="good" class="max-w-screen-xl px-2 mx-auto 2xl:px-0">
      <div class="lg:grid lg:grid-cols-2 lg:gap-4 xl:gap-8">
        <div class="shrink-0 max-w-md lg:max-w-lg mx-auto">
          <div class="flex relative border-2 rounded-lg p-2">
            <img :src="setActive() || useAsset('nophoto.jpg')" class="aspect-3/2 object-cover" />
          </div>

          <div :key="key" class="flex justify-left items-center mt-5 gap-2 flex-wrap">
            <img v-for="(photo, i) in filteredPhotos" :key="i" :src="photo.URL"
              class="max-w-[122px] max-h-[120px] rounded-lg cursor-pointer" :class="{ active: photo.active }"
              @click="changePhoto(i)" />
          </div>
        </div>

        <div class="mt-6 sm:mt-8 lg:mt-0 sm:justify-center sm:px-1 xs:justify-center xs:px-4">
          <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">
            {{ good.marketNAME }}
          </h1>
          <div class="mt-3 sm:items-center sm:gap-3 sm:flex">
            <p class="text-2xl font-extrabold text-gray-900 sm:text-3xl dark:text-white">
              {{ formatPrice(good.priceCLIENT) }}
            </p>
          ...
          <div class="mt-6 sm:gap-4 sm:items-center sm:flex sm:mt-8">
            ...
            <a
              ...
              role="button">
              <svg class="w-5 h-5 -ms-2 me-2" aria-hidden="true" width="24"
            ...
            <button
              @click="addData(good, setActive() || useAsset('nophoto.jpg'), qty)"
              :disabled="formatSklad(good) === 0"
              class="text-white mt-4 sm:mt-0 disabled:cursor-not-allowed disabled:bg-gray-300 bg-sky-500 hover:bg-sky-600 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-sky-500 dark:hover:bg-sky-600 focus:outline-none dark:focus:ring-sky-700 flex items-center justify-center">
              <svg class="w-5 h-5 -ms-2 me-2" aria-hidden="true" width="24"
                height="24" fill="none" viewBox="0 0 24 24">
                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
const { photos, getPhotos } = useMarketStore();
...
const filteredPhotos = photos
  .filter((photo) => photo.JIRA_MARKET_ID === +route.params.id)
  .map((photo, i) => {
    return { ...photo, active: i === 0 ? true : false };
  });
...
async function changePhoto(index) {
  filteredPhotos.forEach((photo, i) => {
    ...
    else filteredPhotos[i].active = false;
  });
  key.value++;
}

function setActive() {
  return filteredPhotos.find((photo) => photo.active)?.URL;
}
</script>
